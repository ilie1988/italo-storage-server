...
static void set_page_count(sqlite3* db) {
    ...
    auto cb = [](void* a_param, int argc, char** argv, char** column) -> int {
        if (argc == 0) {
            ITALO_LOG(error, "Failed to set the page count limit");
            return 0;
        }
        ...
        if (res == 0) {
            ITALO_LOG(error, "Failed to convert page limit ({}) to a number",
                     argv[0]);
            return 0;
        }

        ITALO_LOG(info, "DB page limit is set to: {}", res);
    ...
    if (rc) {
        if (errMsg) {
            ITALO_LOG(error, "Query error: {}", errMsg);
        }
    }
}
...
static void check_page_size(sqlite3* db) {
    ...
    auto cb = [](void* a_param, int argc, char** argv, char** column) -> int {
        if (argc == 0) {
            ITALO_LOG(error, "Could not get DB page size");
        }

        int res = strtol(argv[0], NULL, 10);

        if (res == 0) {
            ITALO_LOG(error, "Failed to convert page size ({}) to a number",
                     argv[0]);
            return 0;
        }

        if (res != DB_PAGE_SIZE) {
            ITALO_LOG(warn, "Unexpected DB page size: {}", res);
        } else {
            ITALO_LOG(info, "DB page size: {}", res);
        }
...
bool Database::get_message_count(uint64_t& count) {
    ...
    while (true) {
        ...
        if (rc == SQLITE_BUSY) {
            ...
            ITALO_LOG(critical, "Could not execute `count` db statement");
            break;
        }
    }
    ...
    if (rc != SQLITE_OK) {
        ITALO_LOG(critical, "sqlite reset error: [{}], {}", rc,
                 sqlite3_errmsg(db));
        success = false;
    }
    ...
    while (true) {
        ...
        if (rc == SQLITE_BUSY) {
            ...
            if (db_full_counter % DB_FULL_FREQUENCY == 0) {
                ITALO_LOG(error, "Failed to store message: database is full");
                ++db_full_counter;
            }
            break;
        } else {
            ITALO_LOG(critical, "Could not execute `store` db statement, ec: {}",
                     rc);
            break;
        }
    }
