...
private_key_t italodKeyFromHex(const std::string& private_key_hex) {
    if (private_key_hex.size() != KEY_LENGTH * 2)
        throw std::runtime_error("Italod key data is invalid: expected " +
...
std::string key_to_string(const std::array<uint8_t, italo::KEY_LENGTH>& key) {
    auto pk = reinterpret_cast<const char*>(&key);
    return std::string{pk, italo::KEY_LENGTH};
}
