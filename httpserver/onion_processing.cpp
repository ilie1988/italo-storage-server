...
process_ciphertext_v1(const ChannelEncryption<std::string>& decryptor,
    ...
    try {
        ...
        ITALO_LOG(debug, "Error decrypting an onion request: {}", e.what());
        return ProcessCiphertextError::INVALID_CIPHERTEXT;
    }

    ITALO_LOG(debug, "onion request decrypted: (len: {})", plaintext.size());
    ...
    try {
        ...
        if (inner_json.find("body") != inner_json.end()) {
            ...
            ITALO_LOG(debug, "Found body: <{}>", body);
        ...
        ITALO_LOG(debug, "Error parsing inner JSON in onion request: {}",
                 e.what());
...
process_ciphertext_v2(const ChannelEncryption<std::string>& decryptor,
    ...
    try {
        ...
        if (inner_json.find("headers") != inner_json.end()) {
            ...
            ITALO_LOG(trace, "Found body: <{}>", parsed.ciphertext);
...
static auto gateway_timeout() -> italo::Response {
    return italo::Response{Status::GATEWAY_TIMEOUT, "Request time out"};
}

static auto make_status(std::string_view status) -> italo::Status {
...
static void relay_to_node(const ServiceNode& service_node,
                          ...
                          std::function<void(italo::Response)> cb, int req_idx,
                          bool v2) {
    ...
    if (!dest_node) {
        ...
        ITALO_LOG(warn, "{}", msg);
        auto res = italo::Response{Status::BAD_GATEWAY, std::move(msg)};
        cb(std::move(res));
        return;
    }
    ...
    auto on_response = [cb, &service_node](bool success,
        ...
        if (!success) {
            ITALO_LOG(debug, "[Onion request] Request time out");
            cb(gateway_timeout());
            return;
        }
        ...
        if (data.size() != 2) {
            ITALO_LOG(debug, "[Onion request] Incorrect number of messages: {}",
                     data.size());
            cb(italo::Response{Status::INTERNAL_SERVER_ERROR,
        ...
        if (data[0] != "200") {
            ITALO_LOG(debug, "Onion request relay failed with: {}", data[1]);
        }
        cb(italo::Response{make_status(data[0]), std::move(data[1])});
    };

    ITALO_LOG(debug, "send_onion_to_sn, sn: {} reqidx: {}", *dest_node, req_idx);
...
void RequestHandler::process_onion_req(const std::string& ciphertext,
    ...
    if (!service_node_.snode_ready()) {
        ...
        cb(italo::Response{Status::SERVICE_UNAVAILABLE, std::move(msg)});
        return;
    }

    ITALO_LOG(debug, "process_onion_req, v2: {}", v2);
    ...
    if (const auto info = std::get_if<FinalDesitnationInfo>(&res)) {
        ...
        ITALO_LOG(debug, "We are the final destination in the onion request!");

        this->process_onion_exit(
            ephem_key, info->body,
            [this, ephem_key, cb = std::move(cb)](italo::Response res) {
        ...
        ITALO_LOG(debug, "We are to forward the request to url: {}{}",
        ...
        if ((target.rfind("/lsrpc") == target.size() - 6) &&
            ...
            auto res = italo::Response{Status::BAD_REQUEST, "Invalid url"};
        ...
        case ProcessCiphertextError::INVALID_CIPHERTEXT: {
            ...
            cb(italo::Response{Status::BAD_REQUEST, "Invalid ciphertext"});
            break;
        }
        case ProcessCiphertextError::INVALID_JSON: {
            auto res = italo::Response{Status::BAD_REQUEST, "Invalid json"};
        ...
        ITALO_LOG(error, "UNKNOWN VARIANT");
    }
}
