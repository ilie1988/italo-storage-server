...
/// How long to wait until reporting unreachable nodes to Italod
...
bool reachability_records_t::should_report_as(const sn_pub_key_t& sn,
    ...
    ITALO_LOG(trace, "should_report_as");
    ...
    if (type == ReportType::GOOD) {
        ...
        ITALO_LOG(debug, "[reach] First time failed {} minutes ago",
                 elapsed_min);

        if (it->second.reported) {
            ITALO_LOG(debug, "[reach]  Already reported node: {}", sn);
            // TODO: Might still want to report as unreachable since this status
            // gets reset to `true` on Italod restart
            return false;
        } else if (elapsed > UNREACH_GRACE_PERIOD) {
            ITALO_LOG(debug, "[reach] Will REPORT {} to Italod!", sn);
            return true;
        } else {
...
void reachability_records_t::check_incoming_tests(time_point_t reset_time) {
    ...
    ITALO_LOG(debug, "Last reset or pinged via http: {}s", http_elapsed.count());

    if (http_elapsed > MAX_TIME_WITHOUT_PING) {

        if (would_warn) {
            if (latest_incoming_http_.time_since_epoch() == 0s) {
                ITALO_LOG(warn, "Have NEVER received http pings!");
            } else {
                ITALO_LOG(warn,
            ...
            ITALO_LOG(warn, "Please check your http port. Not being reachable "
        ...
        this->http_ok = true;
        ITALO_LOG(info, "Http port is back to OK");
    }
    ...
    ITALO_LOG(debug, "Last reset or pinged via lmq: {}s", lmq_elapsed.count());
    ...
    if (lmq_elapsed > MAX_TIME_WITHOUT_PING) {
        ...
        if (would_warn) {
            ...
            if (latest_incoming_lmq_.time_since_epoch() == 0s) {
                ITALO_LOG(warn, "Have NEVER received lmq pings!");
            } else {
                ITALO_LOG(
                    warn,
            ...
            ITALO_LOG(warn, "Please check your lmq port. Not being reachable "
        ...
        this->lmq_ok = true;
        ITALO_LOG(info, "Lmq port is back to OK");
    }
}
...
void reachability_records_t::record_reachable(const sn_pub_key_t& sn,
    ...
    ITALO_LOG(trace, "record_reachable");
    ...
    if (no_record) {
        ...
        if (val) {
            ...
            ITALO_LOG(debug, "[reach] Node is reachable via {} (no record) {}",
        ...
        if (type == ReachType::HTTP) {
            ITALO_LOG(debug, "[reach] node {} is {} via HTTP", sn,
                     val ? "OK" : "UNREACHABLE");
            record.http_ok = val;
        } else if (type == ReachType::ZMQ) {
            ITALO_LOG(debug, "[reach] node {} is {} via ZMQ", sn,
                     val ? "OK" : "UNREACHABLE");
            record.zmq_ok = val;
        }
...
bool reachability_records_t::expire(const sn_pub_key_t& sn) {
    ...
    if (erased)
        ITALO_LOG(debug, "[reach] Removed entry for {}", sn);
...
std::optional<sn_pub_key_t> reachability_records_t::next_to_test() {
    ...
    if (it == offline_nodes_.end()) {
        ...
        ITALO_LOG(debug, "Selecting to be re-tested: {}", it->first);
