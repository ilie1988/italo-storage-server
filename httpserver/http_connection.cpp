...
static constexpr auto ITALO_FILE_SERVER_TARGET_HEADER =
    "X-Italo-File-Server-Target";
static constexpr auto ITALO_FILE_SERVER_VERB_HEADER = "X-Italo-File-Server-Verb";
static constexpr auto ITALO_FILE_SERVER_HEADERS_HEADER =
    "X-Italo-File-Server-Headers";
...
// ======================== Italod Client ========================
ItalodClient::ItalodClient(boost::asio::io_context& ioc, std::string ip,
                         uint16_t port)
    : ioc_(ioc), italod_rpc_ip_(std::move(ip)), italod_rpc_port_(port) {}

void ItalodClient::make_italod_request(std::string_view method,
                                     const nlohmann::json& params,
                                     http_callback_t&& cb) const {

    make_custom_italod_request(italod_rpc_ip_, italod_rpc_port_, method, params,
                              std::move(cb));
}

void ItalodClient::make_custom_italod_request(const std::string& daemon_ip,
...
ItalodClient::wait_for_privkey() {
    // fetch SN private key from italod; do this synchronously because we can't
    // finish startup until we have it.
    italo::private_key_t private_key;
    italo::private_key_ed25519_t private_key_ed;
    italo::private_key_t private_key_x;
    ITALO_LOG(info, "Retrieving SN key from italod");
    boost::asio::steady_timer delay{ioc_};
    std::function<void(italo::sn_response_t && res)> key_fetch;
    key_fetch = [&](italo::sn_response_t res) {
        try {
            if (res.error_code != italo::SNodeError::NO_ERROR)
                throw std::runtime_error(italo::error_string(res.error_code));
            else if (!res.body)
                ...
                if (!validateHexKey(legacy_privkey) ||
                    ...
                    private_key = italo::italodKeyFromHex(legacy_privkey);
                    // TODO: check that one is derived from the other as a
                    // sanity check?
                    private_key_ed =
                        private_key_ed25519_t::from_hex(privkey_ed);
                    private_key_x = italo::italodKeyFromHex(privkey_x);
                    // run out of work, which will end the event loop
                }
            }
        } catch (const std::exception& e) {
            ITALO_LOG(critical,
                     "Error retrieving SN privkey from italod @ {}:{}: {}.  Is "
                     "italod running?  Retrying in 5s",
                     italod_rpc_ip_, italod_rpc_port_, e.what());

            delay.expires_after(std::chrono::seconds{5});
            delay.async_wait([this,
                              &key_fetch](const boost::system::error_code&) {
                make_italod_request("get_service_node_privkey", {}, key_fetch);
            });
        }
    };
    make_italod_request("get_service_node_privkey", {}, key_fetch);
...
static void accept_connection(boost::asio::io_context& ioc,
    ...
    acceptor.async_accept([&](const error_code& ec, tcp::socket socket) {
        ITALO_LOG(trace, "connection accepted");
        if (!ec) {
            ...
            if (ec == boost::system::errc::too_many_files_open) {
                ITALO_LOG(critical, "Too many open files, aborting");
                abort();
            }
...
void run(boost::asio::io_context& ioc, const std::string& ip, uint16_t port,
    ...
    ITALO_LOG(trace, "http server run");
...
connection_t::connection_t(boost::asio::io_context& ioc, ssl::context& ssl_ctx,
    ...
    ITALO_LOG(trace, "connection_t [{}]", conn_idx);
...
connection_t::~connection_t() {
    ...
    if (stream_.lowest_layer().is_open()) {
        ITALO_LOG(debug, "Client socket should be closed by this point, but "
                        "wasn't. Closing now.");
...
void connection_t::on_handshake(boost::system::error_code ec) {
    ...
    ITALO_LOG(trace, "Open https socket: {}", sockfd);
    get_net_stats().record_socket_open(sockfd);
    if (ec) {
        ITALO_LOG(debug, "ssl handshake failed: ec: {} ({})", ec.value(),
                 ec.message());
        this->clean_up();
        deadline_.cancel();
        return;
...
void connection_t::notify(const message_t* msg) {
    ...
    if (msg) {
        ITALO_LOG(trace, "Processing message notification: {}", msg->data);
...
void connection_t::read_request() {
    ...
    auto on_data = [self = shared_from_this()](error_code ec,
        ...
        ITALO_LOG(trace, "on data: {} bytes", bytes_transferred);
        ...
        try {
            ...
            ITALO_LOG(critical, "Exception caught processing a request: {}",
                     e.what());
...
bool connection_t::validate_snode_request() {
    if (!parse_header(ITALO_SENDER_SNODE_PUBKEY_HEADER,
                      ITALO_SNODE_SIGNATURE_HEADER)) {
        ITALO_LOG(debug, "Missing signature headers for a Service Node request");
        return false;
    }
    const auto& signature = header_[ITALO_SNODE_SIGNATURE_HEADER];
    const auto& public_key_b32z = header_[ITALO_SENDER_SNODE_PUBKEY_HEADER];
    ...
    if (!service_node_.is_snode_address_known(snode_address)) {
        ...
        ITALO_LOG(debug, "Discarding signature from unknown service node: {}",
                 public_key_b32z);
...
void connection_t::process_storage_test_req(uint64_t height,
    ...
    ITALO_LOG(trace, "Performing storage test, attempt: {}", repetition_count_);
    ...
    if (status == MessageTestStatus::SUCCESS) {
        ...
        ITALO_LOG(debug, "Failed storage test, tried {} times.",
                 repetition_count_);
        nlohmann::json json_res;
...
void connection_t::process_blockchain_test_req(uint64_t,
    ...
    ITALO_LOG(debug, "Performing blockchain test");
...
static void print_headers(const request_t& req) {
    ITALO_LOG(info, "HEADERS:");
    for (const auto& field : req) {
        ITALO_LOG(info, "    [{}]: {}", field.name_string(), field.value());
    }
}

void connection_t::process_onion_req_v2() {

    ITALO_LOG(debug, "Processing an onion request from client (v2)");
    ...
    auto on_response = [wself = std::weak_ptr<connection_t>{
                            shared_from_this()}](italo::Response res) {
        ITALO_LOG(debug, "Got an onion response as guard node");
...
void connection_t::process_onion_req_v1() {
    ...
    ITALO_LOG(debug, "Processing an onion request from client (v1)");
...
void connection_t::process_proxy_req() {
    ...
    ITALO_LOG(debug, "[{}] Processing proxy request: we are first hop", req_idx);
    ...
    if (!parse_header(ITALO_SENDER_KEY_HEADER, ITALO_TARGET_SNODE_KEY)) {
        ITALO_LOG(debug, "Missing headers for a proxy request");
        return;
    }

    delay_response_ = true;

    const auto& sender_key = header_[ITALO_SENDER_KEY_HEADER];
    const auto& target_snode_key = header_[ITALO_TARGET_SNODE_KEY];

    ITALO_LOG(debug, "[{}] Destination: {}", req_idx, target_snode_key);
    ...
    auto on_proxy_response =
        [wself = std::weak_ptr<connection_t>{shared_from_this()},
         req_idx](bool success, std::vector<std::string> data) {
            ITALO_LOG(debug, "on proxy response: {}",
            ...
            if (!success) {
                ITALO_LOG(debug, "Proxy response FAILED (timeout), idx: {}",
                         req_idx);
                self->response_.result(http::status::gateway_timeout);
            } else if (data.size() == 2) {
                ITALO_LOG(debug, "Proxy respose with status, idx: {}", req_idx);
                ...
                ITALO_LOG(debug, "PROXY RESPONSE OK, idx: {}", req_idx);
    ...
    if (!sn) {
        ITALO_LOG(debug, "Could not find target snode for proxy: {}",
                 target_snode_key);
        on_proxy_response(false, {});
        return;
    }

    ITALO_LOG(debug, "Target Snode: {}", target_snode_key);

    // Send this request to SN over either HTTP or ITALOMQ
    auto sn_req =
        ss_client::Request{req.body(), {{ITALO_SENDER_KEY_HEADER, sender_key}}};

    ITALO_LOG(debug, "About to send a proxy exit requst, idx: {}", req_counter);
    req_counter += 1;
...
void connection_t::process_file_proxy_req() {
    ...
    ITALO_LOG(debug, "Processing a file proxy request: we are first hop");

    const request_t& original_req = this->request_.get();

    delay_response_ = true;

    if (!parse_header(ITALO_FILE_SERVER_TARGET_HEADER,
                      ITALO_FILE_SERVER_VERB_HEADER,
                      ITALO_FILE_SERVER_HEADERS_HEADER)) {
        ITALO_LOG(error, "Missing headers for a file proxy request");
    ...
    const auto& target = header_[ITALO_FILE_SERVER_TARGET_HEADER];
    const auto& verb_str = header_[ITALO_FILE_SERVER_VERB_HEADER];
    const auto& headers_str = header_[ITALO_FILE_SERVER_HEADERS_HEADER];

    ITALO_LOG(trace, "Target: {}", target);
    ITALO_LOG(trace, "Verb: {}", verb_str);
    ITALO_LOG(trace, "Headers json: {}", headers_str);

    const json headers_json = json::parse(headers_str, nullptr, false);

    if (headers_json.is_discarded()) {
        ITALO_LOG(debug, "Bad file proxy request: invalid header json");
        response_.result(http::status::bad_request);
        return;
    }
...

    {
        ...
        if (it != original_req.end()) {
            ITALO_LOG(trace, "Content-Type: {}", it->value().to_string());
    ...
    req->target(target);
    req->set(http::field::host, "file.italonet.org");
    ...
    auto cb = [wself = std::weak_ptr<connection_t>{shared_from_this()}](
                  sn_response_t res) {
        ITALO_LOG(trace, "Successful file proxy request!");
        ...
        if (res.raw_response) {
            ...
            ITALO_LOG(trace, "Response: {}", self->response_);
        } else {
            ITALO_LOG(debug, "No response from file server!");
        }

        self->write_response();
    };

    make_https_request(ioc_, "https://file.italonet.org", req, cb);
}
...
void connection_t::process_swarm_req(std::string_view target) {
    ...
    response_.set(ITALO_SNODE_SIGNATURE_HEADER, security_.get_cert_signature());
    ...
    if (target == "/swarms/push_batch/v1") {
        ...
        response_.result(http::status::bad_request);
        ITALO_LOG(trace, "Got storage test request");
        ...
        if (body == nlohmann::detail::value_t::discarded) {
            ITALO_LOG(debug, "Bad snode test request: invalid json");
        ...
        try {
            ...
            response_.result(http::status::bad_request);
            ITALO_LOG(debug, "Bad snode test request: missing fields in json");
            return;
        }

        const auto it = header_.find(ITALO_SENDER_SNODE_PUBKEY_HEADER);
        if (it != header_.end()) {
            const std::string& tester_pk = it->second;
            this->process_storage_test_req(blk_height, tester_pk, msg_hash);
        } else {
            ITALO_LOG(debug, "Ignoring test request, no pubkey present");
        }
    } else if (target == "/swarms/blockchain_test/v1") {
        ITALO_LOG(debug, "Got blockchain test request");
        ...
        try {
            ...
            if (body.find("height") != body.end()) {
                ...
                ITALO_LOG(debug, "No tester height, defaulting to {}", height);
            }
        } catch (...) {
            response_.result(http::status::bad_request);
            ITALO_LOG(debug, "Bad snode test request: missing fields in json");
        ...
        ITALO_LOG(trace, "Received ping_test");
...
void connection_t::set_response(const Response& res) {
    ...
    case ContentType::json:
        ...
        break;
    default:
        ITALO_LOG(critical, "Unrecognized content type");
    }
...
void connection_t::process_request() {
    ...
    ITALO_LOG(debug, "connection_t::process_request");
    ...
    ITALO_LOG(debug, "target: {}", target);

    const bool is_swarm_req = (target.find("/swarms/") == 0);

    if (is_swarm_req) {
        ITALO_LOG(debug, "Processing a swarm request: {}", target);
    }
    ...
    case http::verb::post: {
        ...
        if (!service_node_.snode_ready(&reason)) {
            ITALO_LOG(debug,
                     ...
                     reason);
            ITALO_LOG(debug, "Would send 503 error (2)");
        ...
        if (target == "/storage_rpc/v1") {
            ...
            ITALO_LOG(trace, "POST /storage_rpc/v1");
            ...
            ioc_.post([]() {
                ITALO_LOG(warn, "Sleeping for some time...");
            ...
            ITALO_LOG(debug, "unknown target for POST: {}", target);
            this->body_stream_
    ...
    case http::verb::get:
        ...
        if (target == "/get_stats/v1") {
            ...
            ITALO_LOG(debug, "unknown target for GET: {}", target);
            response_.result(http::status::not_found);
        }
        break;
    default:
        ITALO_LOG(debug, "bad request");
        response_.result(http::status::bad_request);
        break;
    }
}
...
void connection_t::write_response() {
    ...
    ITALO_LOG(trace, "write response, {} bytes", response_.body().size());
    ...
    if (!body_stream.empty()) {
        ...
        if (!response_.body().empty()) {
            ITALO_LOG(debug, "Overwritting non-empty body in response!");
        }
    ...
    http::async_write(
        stream_, response_, [self = shared_from_this()](error_code ec, size_t) {
            if (ec && ec != boost::asio::error::operation_aborted) {
                ITALO_LOG(error, "Failed to write to a socket: {}",
                         ec.message());
            }
...
void connection_t::process_client_req_rate_limited() {
    ...
    ITALO_LOG(trace, "process_client_req_rate_limited");
    ...
    if (rate_limiter_.should_rate_limit_client(client_ip)) {
        this->body_stream_ << "too many requests\n";
        response_.result(http::status::too_many_requests);
        ITALO_LOG(debug, "Rate limiting client request.");
        return;
    }
    ...
    if (req.find(ITALO_LONG_POLL_HEADER) != req.end()) {
        header_[ITALO_LONG_POLL_HEADER] =
            req.at(ITALO_LONG_POLL_HEADER).to_string();
    }

    const bool lp_requested =
        header_.find(ITALO_LONG_POLL_HEADER) != header_.end();
    ...
    if (lp_requested) {
        ITALO_LOG(debug, "Received a long-polling request");
        ...
        delay_timer->async_wait([self = shared_from_this(), delay_timer,
            ...
            self->request_handler_.process_client_req(
                plaintext, [wself = std::weak_ptr<connection_t>{self}](
                    ...
                    ITALO_LOG(debug, "Respond to a long-polling client");
        ...
        request_handler_.process_client_req(
            plain_text, [wself = std::weak_ptr<connection_t>{
                ...
                auto self = wself.lock();
                if (!self) {
                    ITALO_LOG(debug, "Connection is no longer valid, dropping "
                                    "proxy response");
                    return;
                }

                ITALO_LOG(debug, "Respond to a non-long polling client");
                self->set_response(res);
                self->write_response();
            });
    }
...
void connection_t::register_deadline() {
    ...
    deadline_.async_wait([self = shared_from_this()](error_code ec) {
        ...
        if (ec) {
            ITALO_LOG(error, "Deadline timer error [{}]: {}", ec.value(),
                     ec.message());
        }

        ITALO_LOG(debug, "[{}] Closing [connection_t] socket due to timeout",
                 self->conn_idx);
        self->clean_up();
    });
}
...
void connection_t::on_shutdown(boost::system::error_code ec) {
    if (ec == boost::asio::error::eof) {
        ...
        ITALO_LOG(debug, "Could not close ssl stream gracefully, ec: {} ({})",
                 ec.message(), ec.value());
    }

    const auto sockfd = stream_.lowest_layer().native_handle();
    ITALO_LOG(trace, "Close https socket: {}", sockfd);
...
void HttpClientSession::on_connect() {
    ...
    ITALO_LOG(trace, "Open http socket: {}", sockfd);
    get_net_stats().record_socket_open(sockfd);
...
void HttpClientSession::on_write(error_code ec, size_t bytes_transferred) {
    ...
    ITALO_LOG(trace, "on write");
    if (ec) {
        ITALO_LOG(error, "Http error on write, ec: {}. Message: {}", ec.value(),
                 ec.message());
        trigger_callback(SNodeError::ERROR_OTHER, nullptr);
        return;
    }

    ITALO_LOG(trace, "Successfully transferred {} bytes", bytes_transferred);
...
void HttpClientSession::on_read(error_code ec, size_t bytes_transferred) {
    ...
    if (!ec || (ec == http::error::end_of_stream)) {
        ...
        ITALO_LOG(trace, "Successfully received {} bytes.", bytes_transferred);
        ...
        if (http::to_status_class(res_.result_int()) ==
            ...
            ITALO_LOG(error, "Http request failed, error code: {}",
                     res_.result_int());
        ...
        if (ec != boost::asio::error::operation_aborted) {
            ITALO_LOG(error, "Error on read: {}. Message: {}", ec.value(),
                     ec.message());
        }
        trigger_callback(SNodeError::ERROR_OTHER, nullptr);
    }
    ...
    deadline_timer_.async_wait(
        [self = shared_from_this()](const error_code& ec) {
            if (ec) {
                ...
                ITALO_LOG(debug, "client socket timed out");
                self->clean_up();
            }
        });
}
...
void HttpClientSession::trigger_callback(SNodeError error,
                                         std::shared_ptr<std::string>&& body) {
    ITALO_LOG(trace, "Trigger callback");
...
void HttpClientSession::clean_up() {
    ...
    if (!needs_cleanup) {
        ...
        ITALO_LOG(debug, "No need for cleanup");
        return;
    }
    ...
    if (!socket_.is_open()) {
        ...
        ITALO_LOG(critical, "Socket is already closed");
        return;
    }
    ...
    if (ec && ec != boost::system::errc::not_connected) {
        ITALO_LOG(error, "Socket shutdown failure [{}: {}]", ec.value(),
                 ec.message());
    }
    ...
    if (ec) {
        ITALO_LOG(error, "Closing socket {} failed [{}: {}]", sockfd, ec.value(),
                 ec.message());
    } else {
        ITALO_LOG(trace, "Close http socket: {}", sockfd);
        get_net_stats().record_socket_close(sockfd);
    }
}
...
auto parse_combined_payload(const std::string& payload) -> CiphertextPlusJson {
    ...
    ITALO_LOG(trace, "Parsing payload of length: {}", payload.size());

    auto it = payload.begin();

    /// First 4 bytes as number
    if (payload.size() < 4) {
        ITALO_LOG(warn, "Unexpected payload size");
        throw std::exception();
    }
    ...
    ITALO_LOG(trace, "Ciphertext length: {}", n);
    ...
    ITALO_LOG(debug, "ciphertext length: {}", ciphertext.size());

    const auto json_blob = std::string(it + n, payload.end());

    ITALO_LOG(debug, "json blob: (len: {})", json_blob.size());
