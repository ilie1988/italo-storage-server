...
void command_line_parser::parse_args(int argc, char* argv[]) {
    ...
    // clang-format off
    desc_.add_options()
        ("data-dir", po::value(&options_.data_dir), "Path to persistent data (defaults to ~/.italo/storage)")
        ...
        ("italod-rpc-ip", po::value(&options_.italod_rpc_port), "RPC IP on which the local Italo daemon is listening (usually localhost)")
        ("italod-rpc-port", po::value(&options_.italod_rpc_port), "RPC port on which the local Italo daemon is listening")
        ("lmq-port", po::value(&options_.lmq_port), "Port used by ItaloMQ")
    ...
    hidden.add_options()
        ...
        ("italod-key", po::value(&options_.italod_key), "Legacy secret key (test only)")
        ("italod-x25519-key", po::value(&options_.italod_x25519_key), "x25519 secret key (test only)")
        ("italod-ed25519-key", po::value(&options_.italod_ed25519_key), "ed25519 public key (test only)");
    // clang-format on
    ...
    if (options_.testnet && !vm.count("italod-rpc-port")) {
        options_.italod_rpc_port = 38157;
    }
