...
void serialize_message(std::string& res, const T& msg) {
    ...
    ITALO_LOG(trace, "serialized message: {}", msg.data);
}
...
std::vector<message_t> deserialize_messages(const std::string& blob) {
    ...
    while (!slice.empty()) {
        ...
        /// Deserialize PK
        auto pk = deserialize_string(slice, italo::get_user_pubkey_size());
        if (!pk) {
            ITALO_LOG(debug, "Could not deserialize pk");
            return {};
        }
        ...
        if (!hash) {
            ITALO_LOG(debug, "Could not deserialize hash");
            return {};
        }
        ...
        if (!data) {
            ITALO_LOG(debug, "Could not deserialize data");
            return {};
        }
        ...
        if (!ttl) {
            ITALO_LOG(debug, "Could not deserialize ttl");
            return {};
        }
        ...
        if (!timestamp) {
            ITALO_LOG(debug, "Could not deserialize timestamp");
            return {};
        }
        ...
        if (!nonce) {
            ITALO_LOG(debug, "Could not deserialize nonce");
            return {};
        }

        ITALO_LOG(trace, "Deserialized data: {}", *data);

        ITALO_LOG(trace, "pk: {}, msg: {}", *pk, *data);
