...
RequestHandler::RequestHandler(boost::asio::io_context& ioc, ServiceNode& sn,
                               const ItalodClient& italod_client,
                               const ChannelEncryption<std::string>& ce)
    : ioc_(ioc), service_node_(sn), italod_client_(italod_client),
      channel_cipher_(ce) {}
...
Response RequestHandler::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    ITALO_LOG(trace, "Got client request to a wrong swarm");
...
Response RequestHandler::process_store(const json& params) {
    ...
    for (const auto& field : fields) {
        if (!params.contains(field)) {
            ...
            ITALO_LOG(debug, "Bad client request: no `{}` field", field);
            return Response{
                Status::BAD_REQUEST,
    ...
    ITALO_LOG(trace, "Storing message: {}", data);
    ...
    if (data.size() > MAX_MESSAGE_BODY) {
        ITALO_LOG(debug, "Message body too long: {}", data.size());
...
#ifndef DISABLE_POW
    if (!valid_pow) {
        ITALO_LOG(debug, "Forbidden. Invalid PoW nonce: {}", nonce);
    ...
    if (!success) {
        ...
        ITALO_LOG(warn, "Service node is initializing");
        return Response{Status::SERVICE_UNAVAILABLE,
                        "Service node is initializing\n"};
    }

    ITALO_LOG(trace, "Successfully stored message for {}",
             obfuscate_pubkey(pk.str()));
...
Response RequestHandler::process_snodes_by_pk(const json& params) const {
    ...
    if (!params.contains("pubKey")) {
        ITALO_LOG(debug, "Bad client request: no `pubKey` field");
    ...
    ITALO_LOG(debug, "Snodes by pk size: {}", nodes.size());

    const json res_body = snodes_to_json(nodes);

    ITALO_LOG(debug, "Snodes by pk: {}", res_body.dump());
...
Response RequestHandler::process_retrieve(const json& params) {
    ...
    if (!service_node_.retrieve(pk.str(), last_hash, items)) {
        ...
        auto msg = fmt::format(
            ...
            obfuscate_pubkey(pk.str()));
        ITALO_LOG(critical, "{}", msg);

        return Response{Status::INTERNAL_SERVER_ERROR, std::move(msg)};
    }

    if (!items.empty()) {
        ITALO_LOG(trace, "Successfully retrieved messages for {}",
                 obfuscate_pubkey(pk.str()));
    }
...
void RequestHandler::process_client_req(
    ...
    if (method_name == "store") {
        ...
        ITALO_LOG(debug, "Process client request: snodes for pubkey");
        ...
        ITALO_LOG(debug, "Bad client request: unknown method '{}'", method_name);
...
void RequestHandler::process_lns_request(
    std::string name_hash, std::function<void(italo::Response)> cb) {
    ...
    auto on_italod_res = [cb = std::move(cb)](sn_response_t sn) {
        if (sn.error_code == SNodeError::NO_ERROR && sn.body) {
            cb({Status::OK, *sn.body});
        } else {
            cb({Status::BAD_REQUEST, "unknown italod error"});
        }
    };
...
#else
    italod_client_.make_italod_request("lns_names_to_owners", params,
                                     std::move(on_italod_res));
#endif
}
...
void RequestHandler::process_onion_exit(
    ...
    std::function<void(italo::Response)> cb) {

    ITALO_LOG(debug, "Processing onion exit!");
...
void RequestHandler::process_proxy_exit(
    ...
    ITALO_LOG(debug, "[{}] Process proxy exit", idx);
    ...
    try {
        ...
        if (req.find("headers") != req.end()) {
            if (req.at("headers").find(ITALO_LONG_POLL_HEADER) !=
                req.at("headers").end()) {
                lp_used =
                    req.at("headers").at(ITALO_LONG_POLL_HEADER).get<bool>();
            }
        }

    } catch (std::exception& e) {
        auto msg = fmt::format("JSON parsing error: {}", e.what());
        ITALO_LOG(debug, "[{}] {}", idx, msg);
    ...
    if (lp_used) {
        ITALO_LOG(debug, "Long polling requested over a proxy request");
    }

    this->process_client_req(
        body, [this, cb = std::move(cb), client_key, idx](italo::Response res) {
            ITALO_LOG(debug, "[{}] proxy about to respond with: {}", idx,
                     res.status());
...
void RequestHandler::process_onion_to_url(
    ...
    const std::string& payload, std::function<void(italo::Response)> cb) {
    ...
    auto http_cb = [cb = std::move(cb)](sn_response_t res) {
        if (res.error_code == SNodeError::NO_ERROR) {
            cb(italo::Response{Status::OK, *res.body});
        } else {
            ITALO_LOG(debug, "Italo server error: {}", res.error_code);
            cb(italo::Response{Status::BAD_REQUEST, "Italo Server error"});
        }
    };
